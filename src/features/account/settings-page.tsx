...
function getEmailFromLabel(name: string) {
  return `${name.toLowerCase().replace(/\s+/g, '.')}@oiper.com`
}
          ...
          <DialogFooter className="flex-row gap-2 [&>*]:flex-1 sm:[&>*]:flex-none">
            ...
            <ResponsiveDialog.Close asChild>
              <Button
                type="button"
                variant="destructive"
                disabled={confirm !== confirmationText}
              >
                Delete
              </Button>
            </ResponsiveDialog.Close>
...
function ConfigureAccount() {
  ...
  const profile = useMemo(() => {
    ...
    const email = currentUser?.email ?? getEmailFromLabel(label)
    ...
    return {
      ...
      memberSince: formatMemberSince(undefined),
    }
  }, [currentUser])
...
function AccountDangerZone() {
  ...
  return (
    ...
    >
      ...
      <div className="flex">
        <AccountDeleteModal confirmationText={confirmationText}>
